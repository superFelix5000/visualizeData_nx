...
export class BankDataQuery extends QueryEntity<BankDataState> {
  ...
  selectAllFromCurrentBankAccount$: Observable<BankDataEntry[]> = combineLatest(
    [this.selectAll(), this.selectCurrentBankAccount$]
  ).pipe(
  ...
  selectAllEntriesWithMatchedCategories$: Observable<BankDataEntry[]> =
    combineLatest([
      ...
      map(([entries, recipentCategories]) => {
        const rcMap: Map<string, Category> = new Map<string, Category>();
  ...
  selectAllEntriesPerSelectedYear$: Observable<BankDataEntry[]> = combineLatest(
    [this.selectAllEntriesWithMatchedCategories$, this.selectCurrentYear$]
  ).pipe(
    map(([entries, year]) =>
